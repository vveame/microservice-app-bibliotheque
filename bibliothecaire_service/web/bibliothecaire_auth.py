from config import Config
from flask import request, abort
from flask_restx import Namespace, Resource
from service.bibliothecaire_service import BibliothecaireService

api = Namespace('internal', description='Authentication des bibliothecaires')

service = BibliothecaireService()

@api.route('/email/<string:email>')
class BibliothecaireByEmail(Resource):
    def get(self, email):
        api_key = request.headers.get("X-API-KEY")

        if not api_key or api_key != Config.INTERNAL_API_KEY:
            abort(403, "Forbidden: internal access only")

        result = service._get_bibliothecaire_by_email(email)
        if not result:
            abort(404, "Bibliothecaire non trouvé")

        return {
            "userId": result.get("userId") or str(result.get("_id")),
            "email": result["email"],
            "password": result["password"],  # hashed
            "role": "BIBLIOTHECAIRE"
        }